...
class GameEngine {
  ...
  async run() {
    ...
    console.log(
      `I selected a random value in the range 0..1 (HMAC=${fair.getHMAC()})`
    );
    ...
    while (true) {
      ...
      console.log(
        `My selection: ${reveal.computerValue} (KEY=${reveal.secretKeyHex})`
      );
      console.log(
        `${
  ...
  async selectDice(computerFirst) {
    if (computerFirst) {
      ...
      console.log(
        `I make the first move and choose the [${this.diceList[
      ...
      await this.userChooseDice();
      this.computerDiceIndex = this.diceList.findIndex(
        (_, i) => i !== this.userDiceIndex
      );
      console.log(
        `I choose the [${this.diceList[
          this.computerDiceIndex
        ].toString()}] dice.`
      );
    }
  ...
  async userChooseDice() {
    while (true) {
      ...
      if (
        ...
        console.log(
          "Invalid dice selection. Please choose a valid dice number that is not already taken."
        );
        continue;
      }
      this.userDiceIndex = index;
      console.log(
        `You choose the [${this.diceList[this.userDiceIndex].toString()}] dice.`
      );
      break;
    }
  }
  ...
  async playRolls() {
    const compRoll = await this.performFairRoll(
      "my",
      this.diceList[this.computerDiceIndex]
    );
    const userRoll = await this.performFairRoll(
      "your",
      this.diceList[this.userDiceIndex]
    );
  ...
  async performFairRoll(who, dice) {
    ...
    console.log(
      `My number is ${reveal.computerValue} (KEY=${reveal.secretKeyHex}).`
    );
    console.log(
      `The fair number generation result is ${reveal.computerValue} + ${userNum} = ${reveal.result} (mod ${facesCount}).`
    );
